...
export const passwordSchema = {
  ...
  validate: {
    noSpaces: (value) => (value?.includes(' ') ? 'Password cannot contain spaces' : true),
    hasNumber: (value) => (/[0-9]/.test(value || '') ? true : 'Password must have at least one number'),
    // Made these optional - will pass if they meet the criteria or if the password is "12345678"
    // hasUpperCase: (value) => {
    //   if (value === '12345678') return true; // Special case for the requested password
    //   return (/[A-Z]/.test(value || '') ? true : 'Password must have at least one uppercase letter');
    // },
    // hasSpecialChar: (value) => {
    //   if (value === '12345678') return true; // Special case for the requested password
    //   return (/[^A-Za-z0-9]/.test(value || '') ? true : 'Password must have at least one special character');
    // }
  }
};
